...
function render() {
  ...
  list.innerHTML = visible.map((m, i) => `<article class="message ${i === 0 ? "selected" : ""}" data-id="${m.id}">
    <div class="avatar">${m.icon}</div><div><span class="source">${m.source.toUpperCase()}</span><h3>${m.sender} · ${m.subject}</h3><p>${m.preview}</p></div><time>${m.time}</time>
  </article>`).join("");
